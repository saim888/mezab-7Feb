...
new ScrollMagic.Scene({
    ...
    triggerHook: 'onEnter',
    reverse: true // Set to true if you want "out" animation when scrolling back up
})
.setClassToggle(footerSection, 'footer-animated-in') // Add class on enter
.on('leave', function (event) { // Optional: For "out" animation
    if (event.scrollDir === 'DOWN') { // Only animate out when scrolling down
        footerSection.classList.add('footer-animated-out');
        footerSection.classList.remove('footer-animated-in'); // Ensure "in" class is removed
    } else if (event.scrollDir === 'UP') {
        footerSection.classList.remove('footer-animated-out'); // Remove "out" class if scrolling up
    }
})
.on('enter', function(event){ // Ensure "in" class is added back when entering again
    if (event.scrollDir === 'UP'){ // Only if scrolling up into footer
        footerSection.classList.remove('footer-animated-out'); // remove "out" class when entering
        footerSection.classList.add('footer-animated-in'); // Add "in" class again.
    }
})
.addTo(controller);